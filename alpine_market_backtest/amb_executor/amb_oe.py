...
from alpine_market_backtest.amb_auth import amb_cred
...
def candle_data_loop_fun(SHARED_PROCESS_HOLD_FLAG,TIME_FRAME,START_DATETIME,SHARED_TIME_FRAME_FLAG_DICT,SHARED_STOP_REPEAT_BUY_FLAG_DICT,SHARED_CANDLE_DATA_SET_FLAG_DICT,LOCK):
    ...
    oeoio = amb_oei(sql_host=amb_cred.sql_host, sql_user=amb_cred.sql_user, sql_pass=amb_cred.sql_pass,pool_size=32)
    ...
    while(True):
        with LOCK:
            if(SHARED_TIME_FRAME_FLAG_DICT[TIME_FRAME]):
                ...
                with open(fr"amb_logs/candleData_log_{dt_now.day}_{dt_now.month}","a") as fl:
...
def script_creator_fun(SYMBOL,TIME_FRAME,QUANTITY,STAT_MODULE_NAME,SHARED_SCRIPT_PROCESSESS_DICT,SHARED_STOP_REPEAT_BUY_FLAG_DICT,SHARED_CANDLE_DATA_SET_FLAG_DICT,SHARED_ACTIVE_BUIED_PROCESS_IDS,SHARED_SCRIPT_PROCESS_IDS_VALUE,LOCK):
    ...
    oeoio = amb_oei(sql_host=amb_cred.sql_host, sql_user=amb_cred.sql_user, sql_pass=amb_cred.sql_pass,pool_size=2)
    oeo = amb_one_executor(oeoio=oeoio, **para_dict)
    ...
    thsell = threading.Thread(target=oeo.sell_loop,args=(SHARED_STOP_REPEAT_BUY_FLAG_DICT,SHARED_SCRIPT_PROCESS_IDS_VALUE,SHARED_ACTIVE_BUIED_PROCESS_IDS,LOCK))
...
class amb_one_executor:

    def __init__(self,oeoio:amb_oei,SYMBOL:str,TIME_FRAME:str,QUANTITY:int,SHARED_SCRIPT_PROCESS_IDS_VALUE:int,STAT_MODULE_NAME:str,SHARED_STOP_REPEAT_BUY_FLAG_DICT,SHARED_CANDLE_DATA_SET_FLAG_DICT,LOCK):
        ...
        if not isinstance(oeoio,amb_oei): raise AlpineValueError(ConsoleColors.RED +"invalid parameter value for one_executor_off_initi"+ ConsoleColors.RESET)
        self.oeoio=oeoio
        ...
        try:
            module = importlib.import_module(f"alpine_market_backtest.amb_strategies.{self.STAT_MODULE_NAME}")
            stat_class = getattr(module, 'amb_stat')
    ...
    def buy_loop(self,SHARED_STOP_REPEAT_BUY_FLAG_DICT,SHARED_CANDLE_DATA_SET_FLAG_DICT,SHARED_ACTIVE_BUIED_PROCESS_IDS,SHARED_SCRIPT_PROCESS_IDS_VALUE,LOCK):
        ...
        while(True):
            with LOCK:
                if not SHARED_STOP_REPEAT_BUY_FLAG_DICT[self.TIME_FRAME][SHARED_SCRIPT_PROCESS_IDS_VALUE] and not self.BUY_FLAG and SHARED_CANDLE_DATA_SET_FLAG_DICT[self.TIME_FRAME][SHARED_SCRIPT_PROCESS_IDS_VALUE]:
                    STATUS,RESPONSE=self.stat.buy_condition()
                    if STATUS:
                        tokenId=self.adbsoo.get_script_info(self.SYMBOL)["NEOTOKENID"]
                        ltp=self.adbsoo.get_ltp(tokenId)["LTP"]
                    
                        SHARED_ACTIVE_BUIED_PROCESS_IDS.append(SHARED_SCRIPT_PROCESS_IDS_VALUE)
                        self.ACTIVE_TRADE_ID=self.adbsoo.add_new_activeTrade(scname=RESPONSE["TRADESCNAME"])
                        self.BUY_FLAG=True
                        SHARED_STOP_REPEAT_BUY_FLAG_DICT[self.TIME_FRAME][SHARED_SCRIPT_PROCESS_IDS_VALUE]=True
                        self.adbsoo.set_activeTrade_Value(tradeId=self.ACTIVE_TRADE_ID,valueType="BUYP",value=ltp)
                        self.adbsoo.set_activeTrade_Value(tradeId=self.ACTIVE_TRADE_ID,valueType="BUYT",value=datetime.datetime.now())
                        ...
                        self.adbsoo.set_activeTrade_Value(tradeId=self.ACTIVE_TRADE_ID,valueType="ALPINETYPE",value="amb")
                        ...
                        print("............SETTED BUIED VALUES............")
                                 
                    SHARED_CANDLE_DATA_SET_FLAG_DICT[self.TIME_FRAME][SHARED_SCRIPT_PROCESS_IDS_VALUE]=False
            time.sleep(0.02)
            
    def sell_loop(self,SHARED_STOP_REPEAT_BUY_FLAG_DICT,SHARED_SCRIPT_PROCESS_IDS_VALUE,SHARED_ACTIVE_BUIED_PROCESS_IDS,LOCK):
        print("............SELL LOOP............")
        while(True):
            if self.BUY_FLAG:
                ...
                if STATUS:
                    ...
                    self.adbsoo.set_activeTrade_Value(tradeId=self.ACTIVE_TRADE_ID,valueType="SELLP",value=ltp)

                    self.adbsoo.set_activeTrade_Value(tradeId=self.ACTIVE_TRADE_ID,valueType="SELLT",value=datetime.datetime.now())
                    ...
                    self.adbsoo.set_trade_report(year=tnt.year,month=tnt.month,day=tnt.day,alpineType="amb",scSymbol=self.stat.trade_symbol_selector(),buyT=td_report['BUYT'],timeFrame=self.TIME_FRAME,statName=self.STAT_MODULE_NAME,quantity=self.QUANTITY,scBuyp=td_report['BUYP'],sellT=td_report['SELLT'],scSellP=td_report['SELLP'],scPal=td_report['SELLP']-td_report['BUYP'],scHighP=None,scPalH=None,scLowP=None,scPalL=None,isOption=False)
